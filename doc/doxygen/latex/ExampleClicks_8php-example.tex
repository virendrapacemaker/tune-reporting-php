...
use \hyperlink{classTune_1_1Management_1_1Reports_1_1ReportReaderCSV}{Tune\(\backslash\)Management\(\backslash\)Reports\(\backslash\)ReportReaderCSV};
\textcolor{comment}{}
\textcolor{comment}{/**}
    ...
    \{
        ...
        \textcolor{keywordflow}{try} \{
            ...
            \textcolor{keywordflow}{while} (\textcolor{keyword}{true}) \{
                ...
                $response = $export->download($job\_id);

                \textcolor{keywordflow}{if} (is\_null($response)) \{
                    \textcolor{keywordflow}{throw} new \(\backslash\)Exception(\textcolor{stringliteral}{"No response returned from export request."});
                \}

                $request\_url = $response->getRequestUrl();
                $response\_http\_code = $response->getHttpCode();

                \textcolor{keywordflow}{if} (is\_null($response->getData())) \{
                    \textcolor{keywordflow}{throw} new \(\backslash\)Exception(
                        \textcolor{stringliteral}{"No response data returned from export. Request URL: '\{$request\_url\}'"}
                    );
                \}

                \textcolor{keywordflow}{if} ($response\_http\_code != 200) \{
                    \textcolor{keywordflow}{throw} new \(\backslash\)Exception(
                        \textcolor{stringliteral}{"Service failed request: \{$response\_http\_code\}. Request URL: '\{$request\_url\}'"}
                    );
                \}

                $status = $response->getData()[\textcolor{stringliteral}{"status"}];
                \textcolor{keywordflow}{if} ($status == \textcolor{stringliteral}{"fail"} || $status == \textcolor{stringliteral}{"complete"}) \{
                    \textcolor{keywordflow}{break};
                \}

                $attempt += 1;
                echo \textcolor{stringliteral}{"= attempt: \{$attempt\}, response: "} . print\_r($response, \textcolor{keyword}{true}) . PHP\_EOL;
                sleep(10);
            \}

            \textcolor{keywordflow}{if} ($status != \textcolor{stringliteral}{"complete"}) \{
                \textcolor{keywordflow}{throw} new \(\backslash\)Exception(
                    \textcolor{stringliteral}{"Export request '\{$status\}':, response: "} . print\_r($response, \textcolor{keyword}{true})
                );
            \}

            $report\_url = $response->getData()[\textcolor{stringliteral}{"data"}][\textcolor{stringliteral}{"url"}];

            $csv\_report\_reader = \textcolor{keyword}{new} ReportReaderCSV(
                $report\_url
            );
